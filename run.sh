...
for s in ${RE1_SIZE[@]}; do 
	for a in ${RE1_ASSOC[@]}; do
		...
		./sim ${RE1_BLOCK_SIZE} ${s} ${a} 0 0 0 0 traces/gcc_trace.txt > re/gcc1.${RE1_BLOCK_SIZE}_${s}_${a}.txt
	done
	FA=$(( s/RE1_BLOCK_SIZE ))
		echo "./sim ${RE1_BLOCK_SIZE} ${s} ${FA} 0 0 0 0 traces/gcc_trace.txt > re/gcc1.${RE1_BLOCK_SIZE}_${s}_${FA}.txt"
		./sim ${RE1_BLOCK_SIZE} ${s} ${FA} 0 0 0 0 traces/gcc_trace.txt > re/gcc1.${RE1_BLOCK_SIZE}_${s}_${FA}.txt
done
...
for s in ${RE2_SIZE[@]}; do 
	for a in ${RE2_ASSOC[@]}; do
		...
		./sim ${RE2_BLOCK_SIZE} ${s} ${a} 16384 8 0 0 traces/gcc_trace.txt > re/gcc2.${RE2_BLOCK_SIZE}_${s}_${a}_16384_8.txt
	done
	FA=$(( s/RE2_BLOCK_SIZE ))
		echo "./sim ${RE2_BLOCK_SIZE} ${s} ${FA} 0 0 0 0 traces/gcc_trace.txt > re/gcc2.${RE2_BLOCK_SIZE}_${s}_${FA}_16384_8.txt"
		./sim ${RE2_BLOCK_SIZE} ${s} ${FA} 16384 8 0 0 traces/gcc_trace.txt > re/gcc2.${RE2_BLOCK_SIZE}_${s}_${FA}_16384_8.txt
done
...
for bs in ${RE3_BLOCK_SIZE[@]}; do
	for s in ${RE3_SIZE[@]}; do 
		for a in ${RE3_ASSOC[@]}; do
			...
			./sim ${bs} ${s} ${a} 0 0 0 0 traces/gcc_trace.txt > re/gcc3.${bs}_${s}_${a}_0_0.txt
		done
	done
done
...
for l1 in ${RE4_L1_SIZE[@]}; do 
	for l2 in ${RE4_L2_SIZE[@]}; do
			...
			./sim ${RE4_BLOCK_SIZE} ${l1} ${RE4_L1_ASSOC} ${l2} ${RE4_L2_ASSOC} 0 0 traces/gcc_trace.txt > re/gcc4.${RE4_BLOCK_SIZE}_${l1}_${RE4_L1_ASSOC}_${l2}_${RE4_L2_ASSOC}.txt
	done
done
